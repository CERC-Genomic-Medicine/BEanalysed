...
def plot_roc_auc_per_gene_from_excel_summary(dfs, plotted_value, genes, positive_consequences, negative_consequences, seed=42, out='output.png'):
	...
	for ind in range(0,len(genes)):
		for idx, (y_pred, y_true) in enumerate(gene_data_dict[genes[ind]]):
		...
		axs[ind].grid(False)
	fig.suptitle('Per Gene RAUC analysis', fontsize=16)
	fig.tight_layout()
...
def main():
	...
	plot_roc_auc_per_gene_from_excel_summary(all_sheets, args.plotted_value, list_genes, args.Control_P_Consequences, args.Control_N_Consequences, seed=42, out=args.out)
