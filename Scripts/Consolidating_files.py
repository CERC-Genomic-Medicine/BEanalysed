...
class IsoformFilter:
	...
	def filter_by_isoforms(self, isoforms: Sequence[str]) -> pd.DataFrame:
		...
		if len(filtered['ID']) > len(set(filtered['ID'])):
			...
			id_to_features = ambiguous.groupby("#Uploaded_variation")['Feature'].apply(list).to_dict()
			raise ValueError(
				f"Annotation remains ambiguous after isoform selection, ambiguous guides with ambiguous features:\n{json.dumps(id_to_features, indent=2)}"
			)        
